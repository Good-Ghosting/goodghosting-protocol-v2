...
import { ContractType } from "hardhat/src/internal/hardhat-network/stack-traces/model";
  ...
  it("emits event FundsRedeemedFromExternalPool when redeem is successful", async () => {
    ...
    await expect(result)
      ...
      .withArgs(
        ...
        ethers.BigNumber.from(rewardTokenBalance).sub(ethers.BigNumber.from(adminRewardFeeAmount)),
        ethers.BigNumber.from(governanceTokenRewards).sub(ethers.BigNumber.from(adminGovernanceTokenFee)),
      );
  });
  ...
  it("assign the complete interest to admin", async () => {
    ...
    await expect(result)
      ...
      .withArgs(
        ...
        adminBalance,
        ethers.BigNumber.from(0),
        ethers.BigNumber.from(rewardTokenBalance).sub(ethers.BigNumber.from(adminRewardTokenFee)),
        ethers.BigNumber.from(governanceTokenRewards).sub(ethers.BigNumber.from(adminGovernanceTokenFee)),
      );
  });
  ...
  it("emits Withdrawal event when user withdraws", async () => {
    ...
    await expect(contracts.goodGhosting.connect(player1).withdraw(0))
      ...
      .withArgs(
        ...
        ethers.BigNumber.from(0),
        ethers.BigNumber.from(rewardTokenBalance).sub(ethers.BigNumber.from(adminRewardTokenFee)),
        ethers.BigNumber.from(governanceTokenBalance).sub(ethers.BigNumber.from(adminGovernanceTokenFee)),
      );
  });
  if (strategyType === "curve" || strategyType === "mobius") {
...
export const shouldBehaveLikeAdminWithdrawingFeesFromGGPoolWithFeePercentMoreThan0 = async (strategyType: string) => {
  ...
  it("when funds were not redeemed from external pool", async () => {
    ...
    await expect(contracts.goodGhosting.adminFeeWithdraw()).to.be.revertedWith(
  ...
  it("when admin tries to withdraw fees again", async () => {
    ...
    await expect(contracts.goodGhosting.adminFeeWithdraw()).to.be.revertedWith("ADMIN_FEE_WITHDRAWN()");
  });
  ...
  context("with winners in the game", async () => {
    it("does not revert when there is no interest generated (neither external interest nor early withdrawal fees)", async () => {
      ...
      await contracts.goodGhosting.redeemFromExternalPoolForFixedDepositPool(0);
      const rewardfees = await contracts.goodGhosting.adminFeeAmount(1);
      const governancefees = await contracts.goodGhosting.adminFeeAmount(2);
      ...
      await expect(contracts.goodGhosting.adminFeeWithdraw())
        ...
        .withArgs(
          ...
          ethers.BigNumber.from(0),
          ethers.BigNumber.from(0),
          ethers.BigNumber.from(rewardfees),
          ethers.BigNumber.from(governancefees),
        );
    });
    ...
    it("2 players join the game with different amounts and deposit different amounts at different times throughout and get interest accordingly on withdraw in a curve strategy atricrypto pool", async () => {
      ...
      await expect(result)
        ...
        .withArgs(
          ...
          governanceTokenBalanceDifferenceForPlayer1,
        );
  ...
  it("players join the game and are able to redeem back the funds", async () => {
    ...
    expect(result)
      ...
      .withArgs(
        ...
        expectedInterestValue,
        ethers.BigNumber.from(0),
        ethers.BigNumber.from(rewardTokenBalanceAfterRedeem).sub(ethers.BigNumber.from(adminRewardTokenFee)),
        ethers.BigNumber.from("0"),
      );
  });
