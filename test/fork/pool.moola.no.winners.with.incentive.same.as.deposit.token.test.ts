...
contract("Pool with Moola Strategy with no winners & with extra incentive", accounts => {
  ...
  const players = accounts.slice(1, 5); // 5 players
  ...
  describe("simulates a full game with 5 players and 4 of them winning the game and with admin fee % as 0", async () => {
    ...
    it("fast forward the game", async () => {
      ...
      await timeMachine.advanceTime(parseInt(waitingRoundLength.toString()) * 10);
    });

    it("some ghosts  withdraw from contract", async () => {
      ...
      for (let i = 0; i < players.length / 2; i++) {
        ...
        assert(difference.eq(netAmountPaid), "expected balance diff to be more than paid amount");
      }
    });
    ...
    it("admin withdraws admin fee from contract", async () => {
      if (adminFee > 0) {
        ...
        const inboundTokenPoolbeBalance = web3.utils.toBN(
          await token.methods.balanceOf(goodGhosting.address).call({ from: admin }),
        );
        console.log("bal", inboundTokenPoolbeBalance.toString());
    ...
    it("remaining ghosts withdraw from contract", async () => {
      // starts from 2, since player1 (loser), requested an early withdraw and player 2 withdrew after the last segment
