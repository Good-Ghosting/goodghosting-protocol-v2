...
contract(
  "Pool with Curve Strategy when admin enables early game completion with incentives sent same as deposit token",
  accounts => {
    ...
    describe("simulates a full game with 5 players and 4 of them winning the game and with admin fee % as 0", async () => {
      it("initializes contract instances and transfers Inbound Token to players", async () => {
        ...
        if (configs.deployConfigs.strategy !== "polygon-curve-stmatic-matic") {
          ...
          await token.methods
            .transfer(goodGhosting.address, tokenDecimals.mul(web3.utils.toBN("50")).toString())
      ...
      it("admin withdraws admin fee from contract", async () => {
        if (adminFee > 0) {
          ...
          const adminFeeAmount0 = await goodGhosting.adminFeeAmount(0);
          const adminFeeAmount1 = await goodGhosting.adminFeeAmount(1);
          console.log("adminFeeAmount0", adminFeeAmount0.toString(), "adminFeeAmount1", adminFeeAmount1.toString());
          ...
          if (strategyConfig.gauge !== ZERO_ADDRESS && !rewardokenPoolBalance.isZero()) {
            assert(
              curveRewardBalanceAfter.gt(curveRewardBalanceBefore),
              "expected curve balance after withdrawal to be greater than before withdrawal",
            );
