  ...
  accounts => {
    ...
    describe("simulates a full game with 5 players and 4 of them winning the game and with admin fee % as 0", async () => {
      it("initializes contract instances and transfers Inbound Token to players", async () => {
        ...
        await token.methods
          .transfer(goodGhosting.address, web3.utils.toWei("200").toString())
      ...
      it("admin withdraws admin fee from contract", async () => {
        if (adminFee > 0) {
          ...
          let inboundTokenBalanceBeforeRedeem = web3.utils.toBN(await token.methods.balanceOf(admin).call());
          ...
          let inboundTokenBalanceAfterRedeem = web3.utils.toBN(await token.methods.balanceOf(admin).call());
          ...
          assert(
            inboundTokenBalanceAfterRedeem.gt(inboundTokenBalanceBeforeRedeem),
            "expected token balance after withdrawal to be greater than before withdrawal",
          );
